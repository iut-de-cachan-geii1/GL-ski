...
static int octetHexLire(const char *s)
{
    int n;
    int c = s[0];
...
static int16_t bits16HexLire(const char *s)
{
...
static int lireId(Identificateur *pid, const char *strId)
{
    int k;
    char c;
...
static int serveurEnvoyer(const char *msg)
{
    char *p;
    int n;
    int env;
...
int serveurNomRobot(const char *nom)
{
    char msg[17];
    int n;
...
int serveurUpload(int num, const char *nom)
{
    FILE *f;
    char msg[1024];
    int env, c;
